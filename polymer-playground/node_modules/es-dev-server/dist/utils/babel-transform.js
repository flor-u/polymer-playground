...
exports.polyfillModulesTransform = exports.defaultConfig = void 0;
...
const defaultConfig = {
   ...
   */
  plugins: [require.resolve('@babel/plugin-syntax-dynamic-import'), require.resolve('@babel/plugin-syntax-import-meta'), require.resolve('@babel/plugin-syntax-class-properties'), require.resolve('@babel/plugin-syntax-numeric-separator')],
  sourceType: 'module'
};
/**
...
function createBabelTransform(cfg, baseConfig) {
  const config = _deepmerge.default.all([defaultConfig, baseConfig || {}, // @ts-ignore
  {
...
function createMinCompatibilityBabelTransform(cfg) {
  return createCompatibilityBabelTransform({ ...cfg,
    browserTarget: ['last 2 Chrome major versions', 'last 2 ChromeAndroid major versions', 'last 2 Firefox major versions', 'last 2 Edge major versions', 'last 2 Safari major versions', 'last 2 iOS major versions']
  });
}
/** @param {CreateBabelTransformConfig} cfg */
...
const polyfillModulesTransform = createBabelTransform({}, {
  plugins: [require.resolve('@babel/plugin-proposal-dynamic-import'), require.resolve('@babel/plugin-transform-modules-systemjs'), // systemjs adds template literals, we do systemjs after (potential)
  // es5 compilation so we need to ensure it stays es5
  require.resolve('@babel/plugin-transform-template-literals')],
  babelrc: false,
  configFile: false
});
exports.polyfillModulesTransform = polyfillModulesTransform;