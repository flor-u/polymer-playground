...
function createConfig(config) {
  ...
  let {
    compatibility = _constants.compatibilityModes.AUTO
  } = config;

  if (compatibility === 'modern' || compatibility === 'all') {
    /* eslint-disable-next-line no-console */
    console.warn('[es-dev-server] Compatibility mode "modern" and "all" are deprecated, and combined into "auto".' + '"auto" mode is turned on by default.');
    compatibility = _constants.compatibilityModes.AUTO;
  }

  if (!Object.values(_constants.compatibilityModes).includes(compatibility)) {
  ...
  let appIndexDir; // ensure rootDir is a fully resolved path, for example if you set ../../
  // in the config or cli, it's resolved relative to the current working directory

  rootDir = _path.default.resolve(rootDir); // resolve appIndex relative to rootDir and transform it to a browser path

  if (appIndex) {
    if (_path.default.isAbsolute(appIndex)) {
      appIndex = `/${(0, _utils.toBrowserPath)(_path.default.relative(rootDir, appIndex))}`;
    } else if (!appIndex.startsWith('/')) {
      appIndex = `/${(0, _utils.toBrowserPath)(appIndex)}`;
    } else {
      appIndex = (0, _utils.toBrowserPath)(appIndex);
    }
  ...
  const fileExtensions = [...(fileExtensionsArg || []), ..._buildingUtils.defaultFileExtensions];
  /** @type {boolean | NodeResolveOptions} */

  let nodeResolve = nodeResolveArg; // some node resolve options can be set separately for convenience, primarily
  // for the command line args. we merge them into a node resolve options object

  if (nodeResolveArg != null && nodeResolveArg !== false && (moduleDirs != null || preserveSymlinks != null || dedupeModules != null)) {
    nodeResolve = { // user provided options, if any
    ...
    if (dedupeModules) {
      nodeResolve.dedupe = dedupeModules === true ? importee => !['.', '/'].includes(importee[0]) : dedupeModules;
    }
  }
